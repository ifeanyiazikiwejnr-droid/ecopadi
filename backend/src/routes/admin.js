...
router.post('/products/:id/variants', async (req, res) => {
  const { id } = req.params;
  const { name, value, pricePence, priceDeltaPence, weightGrams } = req.body;
  if (!name || !value) return res.status(400).json({ error: 'Both a name (e.g. "Type") and a value (e.g. "Straight 18\\"") are required.' });
  const productResult = await pool.query('SELECT id FROM products WHERE id = $1', [id]);
  if (!productResult.rows[0]) return res.status(404).json({ error: 'Product not found.' });
  const result = await pool.query(
    `INSERT INTO product_variants (product_id, name, value, price_pence, price_delta_pence, weight_grams) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
    [id, name, value, pricePence ? Math.round(Number(pricePence)) : null, Number(priceDeltaPence) || 0, weightGrams || null]
  );
  res.json(result.rows[0]);
});
...
router.put('/products/:id/variants/:variantId', async (req, res) => {
  ...
  const result = await pool.query(
    `UPDATE product_variants SET name=$1, value=$2, price_pence=$3, price_delta_pence=$4, weight_grams=$5
     WHERE id=$6 AND product_id=$7 RETURNING *`,
    [name, value, pricePence ? Math.round(Number(pricePence)) : null, Number(priceDeltaPence) || 0, weightGrams || null, variantId, id]
  );
